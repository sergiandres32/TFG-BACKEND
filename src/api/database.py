...
def ensure_phase4_topic_secret_schema(engine):
    inspector = inspect(engine)
    table_names = set(inspector.get_table_names())
    ...
    with engine.begin() as conn:
        if "topics" in table_names:
            topic_columns = {col["name"] for col in inspector.get_columns("topics")}
            if "secret_code" not in topic_columns:
                conn.execute(text("ALTER TABLE topics ADD COLUMN secret_code VARCHAR"))
            if "secret_message" not in topic_columns:
                conn.execute(text("ALTER TABLE topics ADD COLUMN secret_message VARCHAR"))

        if "user_topic_secret_unlocks" not in table_names:
            conn.execute(
                text(
                    """
                    CREATE TABLE user_topic_secret_unlocks (
                        id INTEGER PRIMARY KEY,
                        user_id INTEGER NOT NULL,
...
def ensure_phase5_exercise_submission_type_schema(engine):
    inspector = inspect(engine)
    table_names = set(inspector.get_table_names())
    ...
    with engine.begin() as conn:
        if "exercises" in table_names:
            exercise_columns = {col["name"] for col in inspector.get_columns("exercises")}
            if "expected_submission_type" not in exercise_columns:
                conn.execute(
                    text(
                        "ALTER TABLE exercises "
                        "ADD COLUMN expected_submission_type VARCHAR NOT NULL DEFAULT 'c_file'"
                    )
                )
            conn.execute(
                text(
                    "UPDATE exercises "
                    "SET expected_submission_type = 'c_file' "
                    "WHERE expected_submission_type IS NULL"
                )
            )
