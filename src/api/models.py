...
class ExerciseSubmissionType(str, enum.Enum):
    c_file = "c_file"
    zip_makefile = "zip_makefile"
...
class Topic(Base):
    ...
    secret_message = Column(String, nullable=True)
    created_at = Column(DateTime(timezone=True), server_default=func.now())


class UserTopicSecretUnlock(Base):
    __tablename__ = "user_topic_secret_unlocks"
    __table_args__ = (UniqueConstraint("user_id", "topic_id", name="uq_user_topic_secret_unlock"),)
    id = Column(Integer, primary_key=True, index=True)
    user_id = Column(Integer, ForeignKey("users.id", ondelete="CASCADE"), nullable=False, index=True)
...
class Exercise(Base):
    ...
    expected_submission_type = Column(
        Enum(ExerciseSubmissionType),
        default=ExerciseSubmissionType.c_file,
        nullable=False,
    )
